...
def fit_one_epoch(
    ...
    for images, targets in pbar:
        ...
        pbar.set_description(
            f"Training loss: {train_loss.item():.6} | LR: {last_lr:.6}"
        )
...
def main(args):
    ...
    if isinstance(args.val_path, str):
        ...
        val_set = WordGenerator(
            ...
            img_transforms=Compose(
                [
                    T.Resize(
                        (args.input_size, 4 * args.input_size),
    ...
    if args.clearml:
        ...
        task = Task.init(
            project_name="docTR/text-recognition",
            task_name=exp_name,
...
def parse_args():
    ...
    parser.add_argument(
        "--train_path", type=str, default=None, help="path to train data folder(s)"
    )
    parser.add_argument(
        "--val_path", type=str, default=None, help="path to val data folder"
    )
    parser.add_argument(
        "--train-samples",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Minimum number of characters per synthetic sample",
    )
    parser.add_argument(
        "--max-chars",
        type=int,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.001,
        help="learning rate for the optimizer (Adam or AdamW)",
    )
    parser.add_argument(
        "--wd",
        "--weight-decay",
    ...
    parser.add_argument(
        "--vocab", type=str, default="french", help="Vocab to be used for training"
    )
    parser.add_argument(
        "--test-only",
        dest="test_only",
    ...
    parser.add_argument(
        "--clearml", dest="clearml", action="store_true", help="Log to ClearML"
    )
    parser.add_argument(
        "--push-to-hub",
        dest="push_to_hub",
